...
### Three usage modes for end users of generated skills

`[scholar]-perspective` skills generated by scholar-wendao are open-source and shared via git — but **heavy materials (PDFs) always stay on each user's own machine and never circulate through git**. So one scholar skill installed on different machines means different users have their own Library copies (possibly empty, partial, or complete — depending on how deep they want to go):

| Mode | Audience | What to do | PDFs needed? |
|---|---|---|---|
| **1. Lens mode (most common)** | Anyone who wants to use a scholar's perspective on their own material | Just `git clone` the skill | ❌ No |
| **2. Academic verification** | Writers who need to verify citations | Read `references/research/07-archive.md` for DOIs/OA links; verify manually | ❌ No (bibliography includes DOIs/OA links) |
| **3. Deep archive** | Researchers who want the scholar's full library | Install scholar-wendao + skill, run `download_open_access.sh` and `annas_acquire.py` | ✅ Auto-downloaded to your own `$SCHOLAR_WENDAO_LIBRARY` |

**Mode 3 example** (assuming stiegler-perspective is installed):

```bash
# 1. Configure your own Library path (if not set)
...
**Why this design:**
- ✅ Skill repo always clean (legal-clean, lightweight)
- ✅ Each user's Library is reusable across scholars
- ✅ Compatible with any existing academic archive system
- ✅ Legal responsibility is clear (what's on each person's own machine is their own concern)
...
> ⚠️ **License change (2026-05-19)**: This project relicensed from MIT to CC BY-NC 4.0. Versions prior to this commit (`67bc174` and earlier) remain under MIT — those versions and their derivatives retain their original commercial-use rights. **From this commit onwards, commercial use is prohibited.**

Use freely for non-commercial purposes — modify, distribute, fork. If you publicly use perspective skills generated by this tool in academic research, please cite scholar-wendao in your methodology section.

### Commercial Use

This skill is licensed under CC BY-NC 4.0 — **non-commercial use only** (academic research, teaching, personal projects, open-source derivatives).

For commercial licensing inquiries (embedding in paid products, paid consulting use, commercial SaaS integration), contact the author for a commercial license:

📮 **shencong916@gmail.com** (Shen Cong · Tianyu Vision)

The author retains the right to grant commercial licenses on a case-by-case basis.
...
CC BY-NC 4.0 © 2026 Shen Cong · Inspired by [Huashu's nuwa-skill](https://github.com/alchaincyf/nuwa-skill)
